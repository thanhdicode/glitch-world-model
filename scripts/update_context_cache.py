...
- World of Bugs expansion is still closed for training/evaluation; local `WOB-P0` remains blocked
  on missing tar files, but the Kaggle-native `WOB-P0` one-section notebook path is now ready.
...
| WOB expansion | LOCAL_WOB_P0_STATUS=BLOCKED_MISSING_INPUTS; KAGGLE_NATIVE_STATUS=READY_FOR_KAGGLE_WOB_P0; WOB_P1_TRAINING_STATUS=NOT_STARTED | local audit doc plus Kaggle-listing/tooling prep confirm authoritative Kaggle coverage for all 120 non-locked rows and a split-filtered notebook path without local 63 GiB acquisition | run the Kaggle-native `WOB-P0` audit in a notebook with the official datasets mounted; review the resulting audit before any training request | No cross-game, action-conditioning, WOB-result, or locked-test claim yet. |
...
- Current phase: Kaggle-native `WOB-P0` preparation complete; local WOB replay remains blocked.
- Next phase: run the Kaggle-native `WOB-P0` audit in a notebook with official Kaggle datasets
  mounted, without opening WOB training or locked test.
