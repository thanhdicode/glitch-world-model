...
- Phase B / `R5-XGame` is the active mandatory scientific gate, but no metric is claim-ready until
  the downloaded tarball, SHA256 sidecar, and log pass local intake validation.
...
- The Kaggle-native `WOB-P0` pass is verified, `WOB-P1` seed42/seed43/seed44 training artifact
  verification is complete, and Phase B / `R5-XGame` still requires validated output-bundle
  intake before any metric or paper claim update.
...
| WOB expansion | LOCAL_WOB_P0_STATUS=BLOCKED_MISSING_INPUTS; WOB_P0_KAGGLE_STATUS=PASSED; WOB_STATUS=R5_WOB_VALIDATED_POSITIVE_PROBE; WOB_P1_TRAINING_STATUS=SEED44_VALIDATED; WOB_EVALUATION_STATUS=R5_XGAME_ACTIVE_PENDING_INTAKE | verified WOB-P0 bundle confirms 120/120 non-locked rows resolved and 59 locked rows skipped; verified WOB-P1 seed42/seed43/seed44 bundles confirm train-normal / validation-normal training artifacts with false locked-test flags; R5-WOB intake is validated as positive-probe only; R5-XGame runner/validator bundle is implemented | validate the downloaded R5-XGame tarball, SHA256 sidecar, and log before any metric summary | No binary-benchmark, cross-game, action-conditioning, R5-XGame/WOB performance metric, or locked-test claim yet. |
...
- Current phase: Phase B / `R5-XGame` is the active mandatory scientific gate; the repository has
  no validated R5-XGame result yet.
- Next phase: verify and ingest the downloaded tarball/sidecar pair before any metric claim.
