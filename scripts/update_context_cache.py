...
def build_project_state(meta: CacheMetadata, root: Path) -> str:
    blocker = "approved v6 run plus validated Kaggle CUDA/resume artifact set"
...
| 5 | partial | package, hardened validator, CPU resume smokes, ready private dataset | {blocker} | resolve blocker and rerun approved CUDA smoke |
...
- Restore the v5 source root, prepare the package/request, and obtain explicit approval.
...
def context_validation_errors(root: Path) -> list[str]:
    ...
    if "| 5 | partial |" not in project_state:
        errors.append("PROJECT_STATE.md must record Gate 5 partial")
