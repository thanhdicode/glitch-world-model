...
- World of Bugs expansion is still closed for evaluation; local `WOB-P0` remains blocked on
  missing tar files, but the Kaggle-native `WOB-P0` pass is now verified and `WOB-P1` seed42 is
  the next prepared training phase.
...
| WOB expansion | LOCAL_WOB_P0_STATUS=BLOCKED_MISSING_INPUTS; WOB_P0_KAGGLE_STATUS=PASSED; WOB_STATUS=READY_FOR_WOB_P1; WOB_P1_TRAINING_STATUS=NOT_STARTED | verified downloaded Kaggle WOB-P0 bundle confirms 120/120 non-locked rows resolved, 59 locked rows skipped, locked test closed, and no performance metrics | prepare the seed42-only WOB-P1 runner and keep evaluation closed until a separate human command | No cross-game, action-conditioning, WOB-result, or locked-test claim yet. |
...
- Current phase: verified Kaggle-native `WOB-P0` pass recorded; local WOB replay remains blocked.
- Next phase: prepare or run the seed42-only `WOB-P1` Kaggle training runner, without opening WOB
  evaluation or locked test.
