...
- World of Bugs expansion is still closed for training/evaluation; `WOB-P0` completed, but the
  current local attached root is missing 110 of 120 non-locked tar rows required by the frozen
  split.
...
| WOB expansion | BLOCKED_MISSING_INPUTS / WOB-P0_COMPLETE | `docs/research/71_wob_p0_dataset_materialization_audit.md` plus the local audit run confirm frozen protocol coverage, metadata-only manifest preview freeze, existing reduced Lance outputs, and incomplete non-locked local tar materialization | provide the full non-locked WOB tar tree, rerun `WOB-P0`, then request a separate explicit execution command | No cross-game, action-conditioning, or WOB-result claim yet. |
...
- Current phase: `WOB-P0` audit completed; WOB remains blocked on missing non-locked inputs.
- Next phase: provide the missing WOB tar inputs and rerun `WOB-P0` without opening WOB or locked
  test.
