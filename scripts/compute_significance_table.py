...
label column (`label` by default; 0/1). The two files must contain the same set
of (group_key, label) rows so the comparison is paired on identical support.
"""
...
def _read_rows(path: Path, label_column: str, group_key: str) -> list[dict[str, Any]]:
            ...
            rows.append(
                {
                    ...
                    "label": int(float(raw[label_column])),
                }
            )
    if not rows:
...
def compute_significance(
    ...
    *,
    ...
    rows_a = _read_rows(lewm_csv, label_column, group_key)
    rows_b = _read_rows(baseline_csv, label_column, group_key)
