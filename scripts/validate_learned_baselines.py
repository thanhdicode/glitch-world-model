...
def _validate_scores_alignment(
    manifest_records: list[ClipRecord], scores_path: Path
) -> dict[str, Any]:
    ...
    for record, row in zip(manifest_records, rows):
        if row["source"] != record.source or row["clip_dir"] != record.clip_dir:
...
def validate_learned_baselines(output_root: Path) -> dict[str, Any]:
    ...
    manifest_path = Path(protocol["manifest_path"])
    split_path = Path(protocol["split_path"])
    clips_root = Path(protocol["clips_root"]) if protocol.get("clips_root") else None
    ...
    if [_record_signature(record) for record in train_manifest_records] != [
        _record_signature(record) for record in partitions.train_normal
    ]:
        raise ValueError(
            "Persisted train_normal_manifest.csv does not match the grouped train-normal partition."
        )
    if [_record_signature(record) for record in validation_manifest_records] != [
        _record_signature(record) for record in partitions.validation
    ]:
        raise ValueError(
    ...
    for name in BASELINE_NAMES:
        ...
        baseline_validations[name] = {
            ...
            "validation_scores": {
                ...
                **_validate_scores_alignment(validation_manifest_records, scores_path),
            },
        }
