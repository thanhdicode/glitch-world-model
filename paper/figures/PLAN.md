...
| Qualitative timeline panel | Results / Discussion | Show one representative buggy window-score trajectory for the best-by-AUROC TempGlitch LeWM row, explicitly as a qualitative-only diagnostic figure | `paper/figures/fig_timeline_example_receipt.json`; generated from pair-disjoint follow-up selection files plus validated R5 raw LeWM window scores with `temporal_metrics_claimed=false` | C-098, C-115 |
