...
temporal glitch sequences~\cite{tempglitch}. We use this result as background motivation for
normal-only episode-level surprise signals rather than as a same-split comparison: our exact
evaluation episodes, prompts, model family, and scoring protocol differ, and this paper does not
claim superiority over VLM detectors.
...
LeWM-Glitch row reaches AUROC 0.5. This split does not test the method's intended temporal
setting: GlitchBench K2 presents static image-level evidence, while LeWM-Glitch is evaluated
above as an episode-level latent prediction error scorer. The non-LeWM AUROC-1.0 rows still share
...
as an honest negative comparison surface rather than a hidden positive result.
