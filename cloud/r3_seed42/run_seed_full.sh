...
if [[ ! -f "$LEWM_OUTPUT_ROOT/preflight_passed.json" ]]; then
  echo "Refusing full R3/R4 seed ${LEWM_SEED} run: preflight_passed.json is missing." >&2
  exit 1
fi
...
LOG_PATH="$LEWM_OUTPUT_ROOT/r3_seed${LEWM_SEED}_run.log"
