...
cd "$LEWM_REPO_ROOT"
mkdir -p "$WOB_OUTPUT_ROOT/wob_seed42" "$WOB_P1_METADATA_ROOT" "$WOB_TRAIN_ROOT"
...
artifact_root = Path(os.environ["WOB_OUTPUT_ROOT"]) / "wob_seed42"
...
(Path(os.environ["WOB_P1_METADATA_ROOT"]) / "wob_seed42_run_config.json").write_text(
