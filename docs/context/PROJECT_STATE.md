...
- Current phase: the first R5-WOB non-locked Kaggle evaluation attempt stopped before producing an
  output bundle. Its SHA256-verified failure-debug archive was empty, so the exact root cause is
  unresolved; commit `2fb87f0c744a35cec3858faf36da52037bcf14a3` preserves diagnostics for retry.
- Next phase: retry the frozen non-locked R5-WOB path, then download, verify, and ingest its output
  bundle before opening R5-XGAME or R6.
