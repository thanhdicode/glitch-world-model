...
- The local K3 preparation path now has prepared non-locked R5-XGame train and validation Lance
  inputs, hash-recorded manifests, and a passing 12-variant dry-run matrix.
- K3 remains scientifically incomplete until the user-operated Kaggle K3 bundle is downloaded and
  passes local intake validation; no SIGReg or action-conditioning claim is allowed yet.
...
- Next phase on this branch: stop at the user-operated Kaggle K3 ablation gate, then validate the
  downloaded controlled-pair bundle locally before adding any K3 scientific claim.
