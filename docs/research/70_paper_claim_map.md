...
| `R5-XGame` intake is now reproducible across LF/CRLF checkouts because the frozen manifest is validated by normalized CSV content, and the final K-B tarball hash is established by the tarball plus `.sha256` sidecar. | allowed now | `docs/research/128_kb_r5_xgame_final_intake_2026_06_29.md`, `docs/research/94_r5_xgame_intake_reconciliation.md`, `docs/research/16_claim_registry.md` | "cross-platform intake reconciliation" | "the old package receipt itself proves the final tarball SHA" |
