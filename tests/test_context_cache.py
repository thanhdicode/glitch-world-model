...
def test_generated_context_points_to_v4_k1_gate(tmp_path: Path):
    _init_repo(tmp_path)
    _write_minimal_repo(tmp_path)
    update_context_cache(tmp_path, refresh_boot=True)
    ...
    assert "Kaggle gate K1" in next_action
    assert "docs/roadmap/MASTER_ROADMAP_LeWM_Glitch_v4.md" in next_action
    assert "K1 requires an external Kaggle notebook run" in next_action
